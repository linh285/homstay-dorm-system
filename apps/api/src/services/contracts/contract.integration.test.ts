...
const users = [
  { id: 'TST-CT-SALE', username: 'tst-ct-sale', role: 'SALE', branchId: branchA },
  { id: 'TST-CT-ACC', username: 'tst-ct-acc', role: 'ACCOUNTANT', branchId: branchA },
  { id: 'TST-CT-MGR', username: 'tst-ct-mgr', role: 'MANAGER', branchId: branchA },
  { id: 'TST-CT-MGR-B', username: 'tst-ct-mgr-b', role: 'MANAGER', branchId: branchB },
] as const;
...
afterAll(async () => {
  ...
  await prisma.contractService.deleteMany({ where: { contract: { depositId } } });
  ...
  await prisma.requestMember.deleteMany({ where: { rentalRequestId: requestId } });
  await prisma.rentalRequest.deleteMany({ where: { id: requestId } });
  await prisma.customer.deleteMany({ where: { id: { in: [repId, ...memberIds] } } });
...
describe('check-in and handover flow', () => {
  it('runs the whole pipeline and activates the contract with occupied beds', async () => {
    ...
    const created = await sale.post(`/api/v1/contracts/from-deposit/${depositId}`);
    expect(created.status).toBe(201);
    const id = created.body.data.id;
    expect(created.body.data.status).toBe('CHECKIN_DRAFT');

    await sale.post(`/api/v1/contracts/${id}/confirm-arrival`).send({}).expect(200);
    ...
    await sale
      ...
      .expect(200);
    await sale.post(`/api/v1/contracts/${id}/submit-eligibility-review`).expect(200);

    // Sale cannot approve residents; cross-branch manager cannot either.
    await sale.post(`/api/v1/contracts/${id}/residents/${memberIds[0]}/approve`).send({}).expect(403);
    await otherManager.post(`/api/v1/contracts/${id}/residents/${memberIds[0]}/approve`).send({}).expect(403);

    await manager.post(`/api/v1/contracts/${id}/residents/${memberIds[0]}/approve`).send({}).expect(200);
    await manager.post(`/api/v1/contracts/${id}/residents/${memberIds[1]}/approve`).send({}).expect(200);
    await manager.post(`/api/v1/contracts/${id}/approve-eligibility`).expect(200);

    // Cannot create initial payment before the paper contract is signed.
    await accountant
      .post(`/api/v1/contracts/${id}/create-initial-payment`)
      .send({ items: [{ type: 'FIRST_RENT', quantity: 1, unitPrice: '6000000.00' }] })
      .expect(409);
    ...
    await accountant
      ...
      .send({
        items: [
          { type: 'FIRST_RENT', description: 'Tiền thuê kỳ đầu', quantity: 1, unitPrice: '6000000.00' },
        ],
      })
      .expect(200);

    // Cannot submit handover before the payment is confirmed.
    await accountant.post(`/api/v1/contracts/${id}/submit-handover`).expect(422);

    await accountant
      .post(`/api/v1/contracts/${id}/record-initial-payment`)
      .send({ amount: '6000000.00', method: 'CASH', paidAt: new Date().toISOString(), externalEvidenceChecked: true })
      .expect(200);
    await accountant.post(`/api/v1/contracts/${id}/confirm-initial-payment`).expect(200);
    const ready = await accountant.post(`/api/v1/contracts/${id}/submit-handover`);
    ...
    await accountant.post(`/api/v1/handovers/${handoverId}/complete`).expect(403);
    ...
    await manager
      ...
      .send({ utilitiesGuided: true, safetyGuided: true, paperHandoverSigned: true })
      .expect(200);

    const completed = await manager.post(`/api/v1/handovers/${handoverId}/complete`);
    expect(completed.status).toBe(200);
    expect(completed.body.data.status).toBe('COMPLETED');

    const contract = await prisma.contract.findUnique({ where: { id } });
    expect(contract?.status).toBe('ACTIVE');
    const allocations = await prisma.bedAllocation.findMany({ where: { depositId } });
    expect(allocations.every((a) => a.allocationType === 'OCCUPIED')).toBe(true);
  });
});
